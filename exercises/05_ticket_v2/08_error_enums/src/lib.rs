...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match t {
        Ok(ticket) => ticket,
        // This is destructuring the enum and getting the string value
        Err(TicketNewError::TitleErr(e)) => panic!("{}",e), 
        Err(TicketNewError::DescriptionErr(e)) => Ticket::new(title, e, status).unwrap(),
    }
}
#[derive(Debug, PartialEq)]
struct Ticket {
...
impl Ticket {
    pub fn new(
        ...
        if description.is_empty() || description.len() > 500 {
            return Err(TicketNewError::DescriptionErr("Description not provided".to_string()));
        }
